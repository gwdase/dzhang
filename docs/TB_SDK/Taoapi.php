...
class Taoapi
{
    ...
    {
        $this->ApiConfig->Version  = $version;
    ...
    public function Send ($mode = 'GET', $format = 'xml')
    {
        ...
        foreach ($tempParam as $key => $value) 
		{
            if (is_array($value)) {
                    if (! empty($value['image'])) {
                        $imagesArray = $this->FormatUserParam($value);
                    }
                    unset($tempParam[$key]);
            }
        ...
        if (! isset($tempParam['api_key'])) {
            ...
            $systemdefault['v'] = $this->ApiConfig->Version.'.0';
    ...
    {
		$Version = 'SignVersion'.$this->ApiConfig->Version;
